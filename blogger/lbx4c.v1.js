(function(){
  function init(){
    ...
    lbx.style.cssText = 'display:none;width:100%;height:100%;position:fixed;top:0;left:0;z-index:9999';
    lbx.innerHTML =
  ...
  // Watch for comment-body elements to appear in the DOM
  var observer = new MutationObserver(function(mutations){
    mutations.forEach(function(m){
      m.addedNodes.forEach(function(node){
        if(node.nodeType === 1){
          if((node.classList && node.classList.contains('comment-body')) ||
             (node.querySelector && node.querySelector('.comment-body'))){
            observer.disconnect();
            init();
          }
        }
